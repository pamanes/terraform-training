...
resource "aws_subnet" "public_subnet1" {
  ...
  tags                    = merge(local.common_tags, { Name = lower("${local.prefix}-public_subnet1") })
}
...
resource "aws_route_table" "app" {
  ...
  tags = merge(local.common_tags, { Name = lower("${local.prefix}-app") })
}
...
resource "aws_security_group" "nginx_sg" {
  ...
  ingress {
    from_port   = var.aws_security_group_ingress_port
    to_port     = var.aws_security_group_ingress_port
    protocol    = "tcp"
...
resource "aws_instance" "nginx1" {
  ...
  instance_type               = local.env.aws_instance_instance_type
  subnet_id                   = aws_subnet.public_subnet1.id
